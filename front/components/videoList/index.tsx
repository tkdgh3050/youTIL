...
const VideoList: FunctionComponent<PropType> = ({ data, clickDeleteVideo, playListId }) => {
  const queryString = useRef<PlayListInVideo>({ playListId, videoId: data.id });
  ...
  return (
    <VideoControlDivWrapper>
      <Link to="/videoView" state={queryString.current}>
        <OverflowSpan>{data.videoName}</OverflowSpan>
      </Link>
      <StyledButton className="danger" onClick={onClickDeleteVideo}>
        삭제
      </StyledButton>
    </VideoControlDivWrapper>
  );
